...
def scan_ports_with_progress() -> Dict[int, str]:
    ...
    with Progress(
        ...
        task = progress.add_task(
            "Auditing local port sockets...", total=len(SCAN_PORTS)
        )
    ...
    if open_ports:
        ...
        console.print(
            "[status.optimal]✓ Port Security Check passed. No leaks found.[/status.optimal]"
        )
...
def run_advanced_ping() -> List[Dict[str, Any]]:
    ...
    for site in CHECK_SITES:
        ...
        console.print(
            f"  [{color}]▪[/{color}] [text.primary]{site['name']:<18}[/text.primary] → "
