...
def test_run_android(environment_service):
    ...
    ), patch("os.environ.get", return_value="en_US.UTF-8"), patch(
...
def test_run_non_android(environment_service):
    ...
    with patch(
        "shutil.which",
        side_effect=lambda x: {"getprop": None, "termux-battery-status": None}.get(x, None),
    ), patch("platform.system", return_value="Darwin"), patch(
        "platform.machine", return_value="x86_64"
    ), patch("platform.release", return_value="22.0.0"), patch(
